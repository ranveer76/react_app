...
export default function Introduction() {
    const { selectBtn, setSelectBtn, setCurrentTheme, showComponent, setShowComponent, signedin, setSelectedData } = useContext(ResumeContext);

    const handleSelectTemplate = () => {
        if(!signedin){
            alert('Please login to select template')
            return
        }
    ...
    return (
        <>
            <Container className='cont' my={{ base: 1.5, md: 16 }} justifyContent={'space-between'} flexDirection={{ base: 'column', md: 'row', sm: 'column' }} display={'flex'} alignItems={'center'} maxW={'7xl'}>
                <Stack
                    width={{ base: '95%', md: '47%' }}
                    textAlign={'center'}
                    align={'center'}
